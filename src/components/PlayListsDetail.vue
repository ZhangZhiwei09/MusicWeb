...
<script>
import {getPlaylistinnerDetail,getPlaylistComments,getPlaySongs} from "../network/request";
export default {
  name: 'playlist',
  data() {
    return {
  ...
  methods: {
    _getPlaylistDetail(){
          getPlaylistinnerDetail(this.id).then(res=>{
              this.playlist=res.playlist
          })
      },
    _getPlaylistComments(){
          getPlaylistComments(this.id,this.limit,(this.pageNum - 1) * this.limit).then(res=>{
            console.log(res);
